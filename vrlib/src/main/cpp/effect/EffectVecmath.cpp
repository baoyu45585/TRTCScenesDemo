...
// vec3
//--------------------------------------------------------------------------------
EffectVector3::EffectVector3( const EffectVector4& vec ) {
    f_[0] = vec.f_[0];
    f_[1] = vec.f_[1];
    f_[2] = vec.f_[2];
}
...
// vec4
//--------------------------------------------------------------------------------
EffectVector4 EffectVector4::operator*( const EffectMatrix4& rhs ) const {
    EffectVector4 out;
...
// Matrix4
//--------------------------------------------------------------------------------
EffectMatrix4::EffectMatrix4() {
    for ( int32_t i = 0; i < 16; ++i ) {
        f_[i] = 0.f;
    }
}

EffectMatrix4::EffectMatrix4( const float* mIn ) {
    for ( int32_t i = 0; i < 16; ++i ) {
        f_[i] = mIn[i];
    }
}

EffectMatrix4 EffectMatrix4::operator*( const EffectMatrix4& rhs ) const {
    EffectMatrix4 ret;
...
EffectVector4 EffectMatrix4::operator*( const EffectVector4& rhs ) const {
    EffectVector4 ret;
...
EffectMatrix4 EffectMatrix4::inverse() {
    EffectMatrix4 ret;
    float det_1;
    float pos = 0;
    float neg = 0;
...
// Misc
//--------------------------------------------------------------------------------
EffectMatrix4 EffectMatrix4::rotationX( const float fAngle ) {
    EffectMatrix4 ret;
    float fCosine, fSine;
...
EffectMatrix4 EffectMatrix4::rotationY( const float fAngle ) {
    EffectMatrix4 ret;
    float fCosine, fSine;
...
EffectMatrix4 EffectMatrix4::rotationZ( const float fAngle ) {
    EffectMatrix4 ret;
    float fCosine, fSine;
...
EffectMatrix4 EffectMatrix4::translation( const float fX, const float fY, const float fZ ) {
    EffectMatrix4 ret;
    ret.f_[0] = 1.0f;
    ret.f_[4] = 0.0f;
    ret.f_[8] = 0.0f;
...
EffectMatrix4 EffectMatrix4::translation( const EffectVector3 vec ) {
    EffectMatrix4 ret;
    ret.f_[0] = 1.0f;
    ret.f_[4] = 0.0f;
    ret.f_[8] = 0.0f;
...
EffectMatrix4 EffectMatrix4::perspective( float width, float height, float nearPlane, float farPlane ) {
    float n2 = 2.0f * nearPlane;
    float rcpnmf = 1.f / (nearPlane - farPlane);

    EffectMatrix4 result;
...
EffectMatrix4 EffectMatrix4::lookAt( const EffectVector3& vec_eye, const EffectVector3& vec_at, const EffectVector3& vec_up ) {
    EffectVector3 vec_forward, vec_up_norm, vec_side;
    EffectMatrix4 result;
...
void EffectMatrix4::newlookAt( float m[16],const EffectVector3& vec_eye, const EffectVector3& vec_at, const EffectVector3& vec_up ) {
    EffectVector3 vec_forward, vec_up_norm, vec_side;
    ...
    m[8] = vec_side.f_[2];
    m[12] = 0;
    m[1] = vec_up_norm.f_[0];
    m[5] = vec_up_norm.f_[1];
    m[9] = vec_up_norm.f_[2];
    m[13] = 0;
    m[2] = vec_forward.f_[0];
    m[6] = vec_forward.f_[1];
    m[10] = vec_forward.f_[2];
    m[14] = 0;
    m[3] = 0;
    m[7] = 0;
    m[11] = 0;
    m[15] = 1.0;
    m[12] += (-vec_eye.f_[0] * m[0]) + (-vec_eye.f_[1] * m[4]) + (-vec_eye.f_[2] * m[8]);
    m[13] += (-vec_eye.f_[0] * m[1]) + (-vec_eye.f_[1] * m[5]) + (-vec_eye.f_[2] * m[9]);
    m[14] += (-vec_eye.f_[0] * m[2]) + (-vec_eye.f_[1] * m[6]) + (-vec_eye.f_[2] * m[10]);
    m[15] += (-vec_eye.f_[0] * m[3]) + (-vec_eye.f_[1] * m[7]) + (-vec_eye.f_[2] * m[11]);
}
