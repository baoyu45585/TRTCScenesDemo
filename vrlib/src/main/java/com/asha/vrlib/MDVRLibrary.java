...
public class MDVRLibrary {
    ...
    private MDVRLibrary(Builder builder) {
        ...
        mTexture.setScreenWrapper(builder.screenWrapper);
        ...
        if (builder.projectionMode == MDVRLibrary.PROJECTION_MODE_PLANE_FULL ||
            ...
            if (mTexture != null) {
                mTexture.setRenderMode(GLTextureView.RENDERMODE_WHEN_DIRTY);
            }
        }else {
            if (mTexture != null) {
                mTexture.setRenderMode(GLTextureView.RENDERMODE_CONTINUOUSLY);
            }
        }
